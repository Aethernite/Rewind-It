...
const { reducer: timesheetReducer, actions } = createSlice({
  ...
  reducers: {
    ...
    deleteActivitySuccess: (state, action) => {
      ...
      state.timesheet.activities = state.timesheet.activities.filter(
    ...
    saveProjectSuccess: (state, action) => {
      ...
      state.timesheet.activities[action.payload.index].project.name =
        action.payload.project;
      state.timesheet.activities[action.payload.index].project.id =
        action.payload.id;
    ...
    saveTaskSuccess: (state, action) => {
      ...
      // state.timesheet.activities
      console.log(action.payload);
      state.timesheet.activities[action.payload.index].task.name =
        action.payload.task;
      state.timesheet.activities[action.payload.index].task.id =
        action.payload.id;
    ...
    saveDaySuccess: (state, action) => {
      ...
      // state.timesheet.activities
      // console.log(action.payload);
      state.timesheet.activities[action.payload.index].timesheetDays[
        action.payload.day
      ...
      let monday = 0;
      state.timesheet.activities.forEach(
        (activity) => (monday += +activity.timesheetDays[0].hours)
      );
      state.mondayTotal = monday;

      let tuesday = 0;
      state.timesheet.activities.forEach(
        (activity) => (tuesday += +activity.timesheetDays[1].hours)
      );
      state.tuesdayTotal = tuesday;

      let wednesday = 0;
      state.timesheet.activities.forEach(
        (activity) => (wednesday += +activity.timesheetDays[2].hours)
      );
      state.wednesdayTotal = wednesday;

      let thursday = 0;
      state.timesheet.activities.forEach(
        (activity) => (thursday += +activity.timesheetDays[3].hours)
      );
      state.thursdayTotal = thursday;

      let friday = 0;
      state.timesheet.activities.forEach(
        (activity) => (friday += +activity.timesheetDays[4].hours)
      );
      state.fridayTotal = friday;

      let saturday = 0;
      state.timesheet.activities.forEach(
        (activity) => (saturday += +activity.timesheetDays[5].hours)
      );
      state.saturdayTotal = saturday;

      let sunday = 0;
      state.timesheet.activities.forEach(
        (activity) => (sunday += +activity.timesheetDays[6].hours)
      );
...
export const addActivity = () => {
  return async (dispatch, getState) => {
    ...
    try {
      ...
      let a = result.activities.map((activity) =>
        activity.timesheetDays.map((day) => {
          day.date = moment(day.date).format("YYYY-MM-DD");
        })
      );
      ...
      dispatch(
        actions.addCurrentTimesheetActivityFailure(
...
export const saveCurrentTimesheet = () => {
  return async (dispatch, getState) => {
    ...
    try {
      const activities = getState().timesheet.timesheet.activities.slice(
        0,
        getState().timesheet.timesheet.activities.length
      );
